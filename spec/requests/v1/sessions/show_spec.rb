      ...
      context 'when response' do
        ...
        context 'contains' do
          ...
          it 'keys responsible for autofinish with default value' do
            expect(json_response['data']['attributes']['autofinish_enabled']).to be false
            expect(json_response['data']['attributes']['autofinish_delay']).to eq 1440
          end

          it 'keys responsible for autoclose with default value' do
            expect(json_response['data']['attributes']['autoclose_enabled']).to be false
            expect(json_response['data']['attributes']['autoclose_at']).to be_nil
          end
        end
      end
    end
