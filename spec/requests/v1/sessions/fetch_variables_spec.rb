...
RSpec.describe 'GET /v1/sessions/:id/variables/(:question_id)', type: :request do
  ...
  context 'Classic sessions' do
    ...
    let!(:questions) do
      [
        create(:question_single, body: { data: [{ payload: '', value: '' }], variable: { name: 'var1' } }, question_group: question_group, position: 1),
        create(:question_multiple,
               body: { data: [{ payload: '', variable: { name: 'im_a_variable', value: '' } }, { payload: '', variable: { name: 'dep', value: '' } },
                              { payload: '', variable: { name: 'rivia', value: '' } }] }, question_group: question_group, position: 2),
        create(:question_grid,
               body: { data: [{ payload: { rows: [{ payload: '', variable: { name: 'is_with_x' } },
                                           ...
                                           columns: [{ payload: '', variable: { value: '1' } },
                                                     { payload: '', variable: { value: '2' } }] } }] }, question_group: question_group, position: 3),
        create(:question_currency, body: { variable: { name: 'what' }, data: [{ payload: '' }] }, question_group: question_group, position: 4)
      ]
    end

    shared_examples 'correct classic session' do |target_size, target_vars_array|
      before { request }
      ...
      it 'returns correct amount of data' do
        expect(json_response['variable_names'].size).to eq(target_size)
      end

      it 'returns correct variable names' do
        expect(json_response['session_variable']).to eq(session.variable)
        expect(json_response['variable_names']).to match_array(target_vars_array)
      end
    end
    ...
    context 'with question filters' do
      context 'without target question id' do
        ...
        it_behaves_like 'correct classic session', 6, %w[x y im_a_variable rivia dep is_with_x]
      end
      ...
      context 'with target question id' do
        ...
        context 'and without type filters' do
          ...
          it_behaves_like 'correct classic session', 4, %w[var1 dep im_a_variable rivia]
        end
      end
      ...
      context 'digit only' do
        ...
        it_behaves_like 'correct classic session', 7, %w[var1 im_a_variable dep rivia is_with_x x y]
      end
    end
  end
  ...
  context 'CAT-MH sessions' do
    ...
    shared_examples 'correct CAT-MH session' do |target_size|
      ...
      it 'returns correct variable names' do
        ...
        expect(json_response['variable_names']).to match_array(expected_cat_vars)
      end
    end
