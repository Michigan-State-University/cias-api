...
RSpec.describe 'PATCH /v1/tlfb/events', type: :request do
  ...
  let(:request) { patch v1_tlfb_event_path(id: event.id), params: params, headers: headers }

  context 'when params are valid' do
    it 'return correct data' do
      request
      expect(json_response['data']['attributes']).to include('name' => 'anniversary')
    end
  ...
  context 'when id is wrong' do
    let(:request) { patch v1_tlfb_event_path(id: 'wrong_id'), params: params, headers: headers }
