...
RSpec.describe 'POST /v1/live_chat/intervention/:id/navigator_setups/participant_links', type: :request do
  let(:user) { create(:user, :admin, :confirmed) }
  let(:intervention) { create(:intervention, :with_navigator_setup, user: user) }
  let(:headers) { user.create_new_auth_token }
  let(:request) do
    post v1_live_chat_intervention_navigator_setups_participant_links_path(id: intervention.id), params: params, headers: headers
  end
