...
RSpec.describe 'DELETE /v1/questions/:question_id/answer_images/:answer_id', type: :request do
  let(:user) { create(:user, :confirmed, :admin) }
  let(:request) { delete v1_question_destroy_answer_image_path(question.id, answer_id), headers: headers }
  ...
  context 'when auth' do
    context 'is invalid' do
      let(:request) { delete v1_question_destroy_answer_image_path(question.id, answer_id) }
  ...
  context 'when response' do
    ...
    context 'is success' do
      ...
      it 'returns question with removed image_id from answer' do
        expect(json_response['data']['type']).to eq('question')
        answer_without_image = json_response['data']['attributes']['body']['data'].find { |a| a['id'] == answer_id }
        expect(answer_without_image['image_id']).to be_nil
      end
    end
  end
end
