...
describe 'PATCH /v1/interventions/:intervention_id/question_groups/:id/questions_change', type: :request do
  let(:request) { patch questions_change_v1_intervention_question_group_path(intervention_id: intervention.id, id: question_group.id), params: params, headers: headers }
  let(:params) do
    {
      question_group: {
  ...
  let!(:question_group)  { create(:question_group, intervention: intervention, questions: []) }
