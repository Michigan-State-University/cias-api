    ...
    context 'when user cloned a session' do
      ...
      it 'origin and outcome same except variable' do
        expect(session_was.except('variable')).to eq(cloned_session.attributes.except('id', 'generated_report_count', 'created_at', 'updated_at', 'position',
                                                                                      'sms_plans_count', 'logo_url', 'formulas', 'settings',
        ...
        expect(cloned_questions_collection.map(&:attributes)).to include(
          include(
            ...
            'formulas' => [{
              ...
              'patterns' => [
                { 'match' => '=7',
                  'target' => [{ 'id' => cloned_questions_collection.second.id, 'type' => 'Question::Classic::Single', 'probability' => '100' }] }
              ]
            }]
          ),
          include(
          ...
          include(
            ...
            'formulas' => [{
              ...
              'patterns' => [
                { 'match' => '=4',
                  'target' => [{ 'id' => cloned_questions_collection.fourth.id, 'type' => 'Question::Classic::Single', 'probability' => '100' }] }
              ]
            }]
          )
        )
        ...
        expect(cloned_questions_collection.map(&:attributes)).to include(
          include(
            ...
            'formulas' => [{
              ...
              'patterns' => [
                { 'match' => '=11',
                  'target' => [{ 'id' => cloned_questions_collection.first.id, 'type' => 'Question::Classic::Single', 'probability' => '100' }] }
              ]
            }]
          ),
          include(
