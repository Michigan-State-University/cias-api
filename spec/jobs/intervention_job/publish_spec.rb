...
describe InterventionJob::Publish, type: :job do
  ...
  describe '#perform' do
    ...
    let(:session) { create :intervention, problem: intervention }
    ...
    context 'if session has participant' do
      let(:invitation) { create(:intervention_invitation, intervention: session) }
