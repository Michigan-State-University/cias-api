...
RSpec.describe V1::ChartStatistics::UnansweredOwningQuestions do
  ...
  describe '.call - the retired strict rule, retained for the gated path' do
    subject { described_class.call(user_session, missing_vars) }

    let!(:q1) { question('q1') }
    let!(:q2) { question('q2') }
    let(:missing_vars) { ['ht1.q1', 'ht1.q2'] }

    context 'when every owning question is unanswered' do
      it 'is true' do
        expect(subject).to be true
      end
    end

    context 'when only ONE owning question is answered' do
      before { answer!(q1, '5') }

      # This is exactly the population the ungated path now admits and this predicate still
      # drops - which is why the caller must use `none_answered?`.
      it 'is still true, unlike none_answered?' do
        expect(subject).to be true
        expect(described_class.none_answered?(user_session, missing_vars)).to be false
      end
    end

    context 'when every owning question was reached (answered or skipped)' do
      before do
        answer!(q1, '5')
        skip!(q2)
      end

      it 'is false - a skip counts as reached here too' do
        expect(subject).to be false
      end
    end
