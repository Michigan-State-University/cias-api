...
RSpec.describe V1::UserSessionScheduleService do
  ...
  let!(:second_session) { create(:session, intervention: intervention, schedule: schedule, schedule_payload: schedule_payload, position: 2) }
  ...
  context 'user session schedule service' do
    ...
    context 'when session has schedule days after fill' do
      ...
      it 'schedules on correct time' do
        expect { described_class.new(user_session).schedule }.to have_enqueued_job(DaysAfterFillScheduleJob)
