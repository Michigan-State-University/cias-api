...
class V1::VariableReferencesUpdate
  class << self
    def call(intervention_id, &block)
      new(intervention_id).call(&block)
    end

    def update_question_variable_references(question_id, old_variable_name, new_variable_name)
    ...
    def update_session_variable_references(session_id, old_session_variable, new_session_variable)
      ...
      call(session.intervention_id) do |service|
        question_variables = service.extract_question_variables_from_session(session)
  ...
  def update_question_variable_cross_session_references(session, old_var, new_var)
    ...
    update_chart_formulas(old_cross_session_pattern, new_cross_session_pattern)
  end
  ...
  def update_session_variable_cross_session_references(session, old_pattern, new_pattern)
    ...
    update_chart_formulas(old_pattern, new_pattern)
  end

  def extract_question_variables_from_session(session)
    self.class.extract_question_variables_from_session(session)
  end
