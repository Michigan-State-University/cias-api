...
class Import::Basic::ReportTemplateService
  ...
  attr_reader :report_template_hash, :session_id, :logo

  def call
    report_template_sections = report_template_hash.delete(:sections)
    report_template = ReportTemplate.create!(report_template_hash.merge({ session_id: session_id, logo: import_file(logo) }))
