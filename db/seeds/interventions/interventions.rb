...
require_relative './/answer_type_handler'
...
class SeedIntervention
  ...
  user_ids.each do |user_id|
    INTERVENTIONS_PER_USER.times do
      ...
      data.append(
        [fake_uuid, intervention_name, user_id, CURRENT_TIME.to_s, INTERVENTION_STATUS.sample, 'anyone', CURRENT_TIME.to_s,
         CURRENT_TIME.to_s, nil, 27, false, 'Intervention', '', nil, nil, nil, nil, 0, true, 'limited', false,
         SESSIONS_PER_INTERVENTION, false]
      )
    ...
    SESSIONS_PER_INTERVENTION.times do
      ...
      data.append(
        [fake_uuid, intervention_id, session_settings, position_counter, 'Pregnancy 1st Trimester', 'after_fill', 1,
         CURRENT_TIME.to_s, session_formulas, CURRENT_TIME.to_s, CURRENT_TIME.to_s, 0, 0, '0', 's123', nil, 144,
    ...
    QUESTIONS_PER_QUESTION_GROUP.times do
      ...
      data.append(
        [fake_uuid, question.type, question_group_id, question.settings.to_s, position, question.title, question.subtitle,
         question.narrator.to_s, nil, question.formulas.to_s, question.body.to_s, CURRENT_TIME.to_s, CURRENT_TIME.to_s, question.original_text.to_s]
      )
  ...
  data = []
  intervention_ids.each do |intervention_id|
    user_ids[0..ANSWERS_PER_QUESTION - 1].each do |user_id|
      fake_uuid = Faker::Internet.unique.uuid

      data.append(
        [fake_uuid, user_id, intervention_id, nil, 3, 'completed',
  ...
  index = 0
  max_index = user_ids.count * session_ids.count
  data = []
  user_ids.each do |user_id|
