...
INTERVENTIONS_PER_RESEARCHER = 5
SESSIONS_PER_INTERVENTION = 3
REPORT_TEMPLATES_PER_SESSION = 4
QUESTION_GROUPS_PER_SESSION = 4
...
class DBSeed
  ...
  NUM_OF_PARTICIPANTS.times do |index|
    create_user('participant')
    p "#{index + 1}/#{NUM_OF_PARTICIPANTS + NUM_OF_RESEARCHERS} users created"
  end

  NUM_OF_RESEARCHERS.times do |index|
    create_user('researcher')
    p "#{NUM_OF_PARTICIPANTS + index + 1}/#{NUM_OF_PARTICIPANTS + NUM_OF_RESEARCHERS} users created"
  end
  ...
  User.limit_to_roles('researcher').ids.each do |researcher_id|
    create_list(:intervention, INTERVENTIONS_PER_RESEARCHER, user_id: researcher_id) do |intervention|
      ...
      create_list(:session, SESSIONS_PER_INTERVENTION, intervention_id: intervention.id) do |session|
        ...
        create_list(:report_template, REPORT_TEMPLATES_PER_SESSION, :participant, summary: Faker::Movies::HowToTrainYourDragon.character,
                                                                                  session_id: session.id) do |report_template|
  ...
  q_groups_from_interv_excluding(type: 'Intervention').each do |question_group|
    branching_index_max += question_group.questions.size
  end

  Intervention.where.not(type: 'Intervention').includes({ sessions: { question_groups: :questions } }).find_each do |intervention|
    session_paths = []
    question_paths = []
    intervention.sessions.each do |session|
      if intervention.type == 'Intervention::FixedOrder'
  ...
  participant_index = 0
  question_count = 0
  q_groups_from_interv_excluding(status: 'draft').each do |question_group|
  ...
  User.limit_to_roles('participant').ids.each do |participant_id|
    Intervention.includes({ sessions: { question_groups: :questions } }).where.not(status: 'draft').find_each do |intervention|
      if intervention.status == 'published'
        user_intervention = create(:user_intervention, :ready_to_start, user_id: participant_id, intervention_id: intervention.id)
      else
          ...
          session.question_groups.each do |question_group|
            question_group.questions.each do |question|
              ...
              create_list(:answer, ANSWERS_PER_QUESTION, type: "Answer::#{question.type.demodulize}",
                ...
                assign_data_to_answer(answer, question) unless question.question_variables.empty?
              end
            end
          end
        end
