...
class RemoveIdFromGridAndThirdPartyQuestionBody < ActiveRecord::Migration[7.2]
  ...
  def down
    say '-- re-add id to body->data[] for Question::ThirdParty'
    third_party_query = <<~SQL.squish
      UPDATE questions
      SET body = jsonb_set(
        body,
        '{data}',
        (
          SELECT jsonb_agg(element || jsonb_build_object('id', gen_random_uuid()::text))
          FROM jsonb_array_elements(body -> 'data') AS element
        )
      )
      WHERE type = 'Question::ThirdParty'
    ...
    say '-- re-add id to body->data[0] and to rows/columns for Question::Grid'
    grid_query = <<~SQL.squish
      UPDATE questions
      SET body = jsonb_set(
        body,
        ...
        (
          SELECT jsonb_agg(
            jsonb_set(
              element || jsonb_build_object('id', gen_random_uuid()::text),
              '{payload}',
              jsonb_set(
                jsonb_set(
                  element -> 'payload',
                  '{rows}',
                  (
                    SELECT jsonb_agg(row_el || jsonb_build_object('id', gen_random_uuid()::text))
                    FROM jsonb_array_elements(element -> 'payload' -> 'rows') AS row_el
                  ),
                  true
                ),
                '{columns}',
                (
                  SELECT jsonb_agg(col_el || jsonb_build_object('id', gen_random_uuid()::text))
                  FROM jsonb_array_elements(element -> 'payload' -> 'columns') AS col_el
                ),
                true
              ),
